...
1. 更新 YAML、生成器、已评审 JSON、executor 和覆盖矩阵。需要泛化用例时显式运行 `-scope=gen_cases`，评审生成结果后再更新 `atk_<op>.json`；`all` 不负责覆盖已评审 JSON。
2. 按目标 SOC 构建并安装当前代码，确认 ATK 调用的是本轮产物。
3. 快速开发期间只运行 `-scope=accuracy`。当前代码、当前用例和当前构建产物全部通过精度，才算一次有效开发迭代；随后可以继续修改和下一次精度验证，不要求每次都跑其它专项测试。
4. 当一个精度通过版本被选为该轮待验收产物时，固定代码、用例和构建产物，运行 `-scope=all` 完成精度、确定性和 mssanitizer，再单独运行 `-scope=performance`。
5. 验收阶段按改动风险补充 ctypes ABI、公开入口、Example/ST 和 NPU CI。
...
`mixed_tolerance_bm` 统一定义。`ATK_GM_INIT_MODE` 默认为 `on`，统一脚本会向
精度任务加入 `--gm_init_flag`；仅在显式设为 `off` 时关闭 GM 初始化：
...
atk node --name npu_dut --backend npu --devices <npu_device_id> \
  ...
  -s <accuracy_start> -e <accuracy_end> --gm_init_flag -to 14400
```
...
atk node --name npu_dut --backend npu --devices <npu_device_id> \
  ...
  task -c ./atk_<op>_perf.json --task performance_device -p ./executor_<op>.py \
...
```bash
atk node --name npu_dut --backend npu --devices <npu_device_id> \
  task -c ./atk_<op>_mss.json -p ./executor_<op>.py --task accuracy_dc \
  --dc_loop_nums 50 -to 3600 -s <determinism_start> -e <determinism_end>
```
...
mssanitizer --tool=memcheck -- \
  atk node --name npu_dut --backend npu --devices <npu_device_id> \
  task -c ./atk_<op>_mss.json -p ./executor_<op>.py --task run \
