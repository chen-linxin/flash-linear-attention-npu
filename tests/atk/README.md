...
精度用例的输入值域选择按《精度对比与定位》的
[`值域校准`](../../docs/agents/reference/精度对比与定位.md#值域校准)执行；最小可归因用例、固定随机
种子、最终 `(case, seed)` 全组合及日常轮换要求按同文件的
[`日常精度看护`](../../docs/agents/reference/精度对比与定位.md#日常精度看护)执行。本节只维护 ATK
用例规模和覆盖维度，不重复定义上述精度规则。
